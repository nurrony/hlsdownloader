...
describe('PlaylistParser', () => {
  describe('isPlaylist()', () => {
    test('should return true for .m3u8 extensions', () => {
      assert.ok(playlistParser.isPlaylist('http://example.com/video.m3u8'));
    });
    ...
    test('should return false for non-playlist extensions', () => {
      assert.equal(playlistParser.isPlaylist('http://example.com/video.mp4'), false);
      assert.equal(playlistParser.isPlaylist('http://example.com/video.ts'), false);
    });
  });
...
/absolute/path/segment3.ts
    ...
    test('should extract absolute URLs and resolve relative paths', () => {
      ...
      assert.equal(results.length, 3, 'Should have extracted 3 segments');

      // Relative path resolution
      assert.equal(results[0], 'https://media.example.com/hls/segment1.ts');

      // Full URL preservation
      assert.equal(results[1], 'https://external.com/segment2.ts');

      // Absolute path resolution (relative to host root)
      assert.equal(results[2], 'https://media.example.com/absolute/path/segment3.ts');
    });
    ...
    test('should filter out all lines starting with # (HLS tags)', () => {
      ...
      assert.equal(hasTags, false, 'No HLS tags should be present in the results');
    });
    ...
    test('should handle empty or whitespace-only lines', () => {
      ...
      assert.equal(results[0], 'https://media.example.com/hls/segment.ts');
    });
  });
});
